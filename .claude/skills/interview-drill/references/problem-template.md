...
3. **Scaffold the project directly into `NNN-area-slug/src/`** (path relative to the active campaign's own repo root — i.e. `campaigns/<active>/NNN-area-slug/src/` from the skill repo's perspective). The scaffold command and toolchain depend entirely on the drill's stack (from the competency map) — there is no default. **Whatever the stack, do not pre-create the target directory before running its scaffold tool** — most scaffolders (Vite included) refuse to write into a non-empty directory and cancel silently rather than erroring loudly; let the tool create it. `src/`'s build artifacts and dependency directories (`node_modules/`, Go's module cache is fine to leave global, Rust's `target/`, etc.) stay gitignored — only real source lives in this repo's history.
   - **If this drill's task will involve tests** — the human writing them, or making shipped ones pass — wire up the stack's standard test runner as part of the scaffold: install it, write its **config file at the project root**, and add a test command to the manifest. Follow the active campaign's `AGENTS.md` **Toolchain** line for which runner this campaign uses, rather than re-deciding per drill. Decide whether the drill needs this when you pick the area in step 1 — not after `problem.md` is written in step 6, by which point the scaffold is already committed. Write the config file **even when the runner has a zero-config mode**: editor test-explorer integrations discover a project by finding a config file (then resolving the runner from that directory's dependencies), so a manifest-only setup runs fine from the CLI while staying invisible in the editor — which is exactly the retrofit this clause exists to prevent.
   - *Worked example (React + TypeScript, Vite):* `npm create vite@latest src -- --template react-ts`, then `npm install`. Enable `eqeqeq: "error"` in the scaffolded linter config (`.oxlintrc.json`'s `rules`, or the ESLint equivalent) as a standing default. The template ships **no** test setup of its own, so a test-bearing drill also needs the runner installed, a runner config file written, and a `test` script added. **Note:** Vite's own template nests a further `src/` inside the project root (its own convention, not this skill's) — the result is `NNN-area-slug/src/src/App.tsx`. Don't try to flatten it away.
   - *Other stacks:* use that ecosystem's own standard scaffold tool and idiomatic starting config (e.g. `go mod init` + a minimal `main.go`, `cargo new`, `swift package init`) — apply the same principles (a real, buildable project, standing linter/format config enabled, test runner wired up when the drill involves tests) rather than copying the Vite specifics literally. Some ecosystems ship a test runner in the toolchain with nothing to install or configure; where that's true, there's nothing to add beyond confirming the command runs.
