...
3. **Scaffold the project directly into `campaigns/<active>/NNN-area-slug/src/`** (help is encouraged here — bootstrapping is not what's tested). The scaffold tool and toolchain depend entirely on this drill's stack, per the competency map — no default (e.g. React+TS via `npm create vite@latest src -- --template react-ts`; other stacks use their own standard scaffold tool). **Don't pre-create the target directory first** — most scaffolders refuse to write into a non-empty directory and cancel silently rather than erroring; let the tool create it. Enable a standing linter/format config as a default for every drill, whatever this stack's equivalent is. **If this drill's task will involve tests** — the human writing them, or making shipped ones pass — wire up the stack's standard test runner here too: install it, write its **config file at the project root**, and add a test command to the manifest. Decide this when you pick the area in step 1, not after the packet is written in step 6. Write the config file even when the runner has a zero-config mode: editor test-explorer integrations discover a project by finding a config file, so a manifest-only setup runs fine from the CLI while staying invisible in the editor. (Vite's own template nests a further `src/` inside the project root — that's Vite's convention, not this skill's; the result is `NNN-area-slug/src/src/App.tsx` for a React drill specifically. Don't try to flatten it away.)
...
7. **If the human reworks the code mid-discussion** (common — a draft score or a follow-up question often prompts the human to go fix something themselves before re-requesting a grade), re-score against the new working tree and re-present, same as any other round of pushback. This can happen more than once. Regardless of how many rounds of drafting/discussion/rework happened, **only one final grade gets persisted per drill** — track revision count as a fact *within* that one grade (see Phase B step 7's "Revisions before final" and the ledger row's Area(s) cell), never as separate "attempt 1"/"attempt 2" grades, drill ids, or ledger rows. The ledger and `grade.md` describe the final accepted state; earlier drafts exist only in the conversation, not as parallel persisted records.
...
7. Save the finalized version to `campaigns/<active>/NNN-area-slug/grade.md`, describing the single final state the human confirmed as fair. If Phase A involved more than one round of code rework before that confirmation, note the revision count as a fact (e.g. a `**Revisions before final:**` line near the top) and, optionally, a short **Process notes** section narrating what changed round to round and what prompted it (this is good material for the study list/strengths — self-driven debugging is a real signal, see the rubric's dimension 8). Do not create multiple "attempt" sections or files for one grading request. Then update `campaigns/<active>/LEDGER.md` per `references/ledger-template.md`'s "How the AI should use it" section — one row per drill, not one row per revision; the ledger tracks weaknesses as status rows tagged with which drills moved them, not growing prose; keep it a scannable index and let `grade.md`/git history hold the detail.
8. **Commit the graded state (in this campaign's own repo) and tag it `NNN-grade-0N`** (N = this grading *pass*'s number, 1-indexed — a new grading pass means the human asked to be graded again after a prior `NNN-grade-0(N-1)` commit already exists, not "how many times the code was reworked before this one grade request was persisted"). **Check for out-of-scope changes first** (see "Path-scoped commits" in `references/problem-template.md`'s Git workflow section) — stage and commit only this drill's path, never the whole tree. Commit message includes the score, e.g. `003: 4.5/5 (decomp/state 5, clarity 4)`. Never amend a prior graded commit; each pass gets its own. There is no separate `solution/` snapshot — the tagged commit itself is the permanent record of that grading pass (`git show NNN-grade-0N:NNN-area-slug/src/App.tsx`, run from inside this campaign's repo, retrieves any file from it — note this path is stack-specific; a Vite/React drill's file would actually sit at `NNN-area-slug/src/src/App.tsx` per Vite's own nested convention, see Setup step 3).
